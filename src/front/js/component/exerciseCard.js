...
const ExerciseCard = ({ exercise, id, target, equipment, bodypart }) => {
  ...
  return (
    ...
    >
      {
        particularExercise.map(item => item.id).includes(id) ? <i className="heart fa-solid fa-heart" style={{ color: "#ff5300", cursor: "pointer" }} onClick={() => actions.removeFavExercise(bodypart, exercise, id)}></i> :  <i className="heart fa-regular fa-heart" style={{ color: "#ff5300", cursor: "pointer" }} onClick={() => actions.addFavExercise(bodypart, exercise, id)}></i>
      }
      <div
